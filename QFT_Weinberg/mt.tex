...
\eq{
    \mathscr{F}_{\beta}{}^+\to-2i\pi e^{-iE_{\beta}t}\int dx\delta(E_{\alpha}-E_{\beta})g(\alpha){T_{\beta\alpha}}^{+}
}  
写开之后我们之前的\cref{eq:phipsirelation}可以展开写成：
\eq{
    \Psi_g^+(t)\to\int d\beta e^{-iE_\beta t}\Phi_\beta\left[g(\beta)-2i\pi\int d\alpha\delta(E_\alpha-E_\beta)g(\alpha)T_{\beta\alpha}^+\right].
}
在这个基础上，我们
